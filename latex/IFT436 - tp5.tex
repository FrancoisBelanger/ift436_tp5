...
Le choix des algorithmes fut en fonction de ce qui a été vu en classe et des algorithme proposé dans l'énoncé. Donc, les algorithmes sont Bor\r{u}vska, Krustal et Prim. Ce sont trois algorithmes de stratégie gloutonne, qui demande en entrée des structures données similaires.\\
\newline
Le seul algorithme dont le code a été trouvé sur Internet, c'est celui de  Kruskal. Quant à Bor\r{u}vka et Prim, aucun code acceptable a été trouvé, donc c'était préférable de les coder. Par code raisonnable dans ce cas-ci, cela se définie par le fait que le code trouvé ne nous demande pas de le modifier trop et que le code est optimal. Une classe DisjointSet a été créé pour les algoritmes Bor\r{u}vska et Krustal, qui est tiré du code de l'algorithme de Kruskal.\\
\newline
