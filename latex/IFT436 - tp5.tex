...
D'abord, la complexité des algorithme devraient être: $O(m\, log\, n)$ pour Bor\r{u}vka, $O(m\, log\, m)$ pour Kruskal et $O(m\, log\, n)$ pour Prim.
...
Pour recueillir les données sur le temps d'exécution versus le nombre de sommet, la librairie \textit{matplotlib} a été importé. Cette librairie permet de faire un graphique des données recueillies. Cela évite de devoir les compiler dans un fichier pour faire un graphique avec un logiciel comme Excel.
\vspace{0.5cm}
