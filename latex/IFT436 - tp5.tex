...
Au début, pour faire le générateur de nombres aléatoires était basé sur une implémentation en C du \textit{Marsenne Twister} \footnote{https://en.wikipedia.org/wiki/Mersenne\_Twister}, un algorithme avec une période de $2^{19937}-1$ fortement testé. Le code du générateur de graphe utilisait la fonction \textit{randint} de Python, qui est une application du \textit{random} de Python qui utilise le \textit{Marsenne Twister}.

\vspace{0.2cm}

Cependant, il a été modifier pour augmenté la vitesse d'exécution.
